...
__fastcall TForm3::TForm3(TComponent* Owner) : TForm(Owner) {
	KeyPreview = true;   //чтобы работало считывание клавиш
}

// ---------------------------------------------------------------------------
void __fastcall TForm3::Image1MouseDown(TObject *Sender, TMouseButton Button,
	TShiftState Shift, int X, int Y) {   // нажатие мыши
...
void __fastcall TForm3::Image1MouseUp(TObject *Sender, TMouseButton Button,
	TShiftState Shift, int X, int Y) {  //нет движения, если координаты за канвой
...
// ---------------------------------------------------------------------------
void __fastcall TForm3::FormCreate(TObject *Sender) {  //создание формы
...
// ---------------------------------------------------------------------------
void __fastcall TForm3::ColorClick(TObject *Sender) {    //выбор цвета
	TColor style;
...
void __fastcall TForm3::ClearPoligon(int DeleteItem) {   //для удаления полигона
	for (int i = 0; i < 2; i++) {
		for (int j = 0; j < 100; j++)
...
void __fastcall TForm3::PoligonFigure() {    //для рисования полигона
...
void __fastcall TForm3::FormKeyDown(TObject *Sender, WORD &Key,
	TShiftState Shift) { //для завершения полилинии до многокгольника
...
void __fastcall TForm3::CenterLabel(int CenterX, int CenterY, int FigureItem) {
	TLabel *Label = new TLabel(this);  //значок центра
	Label->Parent = this;
...
void __fastcall TForm3::BuildLabel(int FirstX, int FirstY, int FigureItem) {
	TLabel *Label = new TLabel(this); //построение значков
	Label->Parent = this;
...
void __fastcall TForm3::info(int FigureItem) { //запись информации в лейблы
...
void __fastcall TForm3::FindCenter(int FigureItem) {    //поиск центра фигуры
	int temp = 0;
...
void __fastcall TForm3::Button1Click(TObject *Sender) {
	if (Item != -1) {       //удаление
		...
		if (!XY[Item].Elipse) {
			Hide(Sender, Item);  //элипс
			ClearPoligon(Item);
		}
		else {
			DeleteLabel(Item);              //все другие фигуры
			Canvas->Brush->Color = clBtnHighlight;
			Canvas->Pen->Color = clBtnHighlight;
			Canvas->Ellipse(XY[Item].XYPoligon[0][0], XY[Item].XYPoligon[1][0],
				XY[Item].XYPoligon[0][1], XY[Item].XYPoligon[1][1]);
...
// ---------------------------------------------------------------------------
void __fastcall TForm3::move(int MoveX, int MoveY) {    //движение
	if (!XY[Item].Elipse) {  //эллипс
...
void __fastcall TForm3::ClearHelp(TPoint *pnt, int FigureItem) {  //помощь зарисовки фигур
	for (int i = 0; i < XY[FigureItem].iX; i++) {
...
void __fastcall TForm3::Hide(TObject *Sender, int FigureItem) {   //зарисовка фигур цветом фона
...
void __fastcall TForm3::DeleteLabel(int FigureItem) {   //удаление лейблов
...
void __fastcall TForm3::Button3Click(TObject *Sender) {   //изменение размеров
	if (StrToFloat(Edit1->Text) > 0 && Item != -1) {
		...
		TPoint *tpn = new TPoint[XY[Item].iX];
		for (int i = 0; i < XY[Item].iX; i++) { //все остальное
...
void __fastcall TForm3::BrushColorClick(TObject *Sender) {  // выбор заливки
	TColor style;
...
void __fastcall TForm3::Button4Click(TObject *Sender) {     //поворот
	int tempAngle = 0;
...
void __fastcall TForm3::infoElipce(int FigureItem) {  //инфа про эллипс
	unsigned int TempX =
...
void __fastcall TForm3::ComponentColor(TObject *Sender, int FigureItem) {
	TColor style;
	switch (XY[FigureItem].BrushColor) {  //выбор заливки
	case 0:
		style = clBtnHighlight;
		break;
	case 1:
	...
	switch (XY[FigureItem].PenColor) {    //выбор цвета пера
	case 0:
		style = clBlack;
		break;
	case 1:
...
void __fastcall TForm3::SaveColor(int FigureItem) {  //сохранение выбранного цвета
...
void __fastcall TForm3::Button2Click(TObject *Sender) { //если нажата кнопка движения
	isThisMove = true;
}
// ---------------------------------------------------------------------------
